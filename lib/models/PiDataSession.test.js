...
describe('tests for the PiDataSession model', () => {
  ...
  it('has a required sensorType field that only accepts the strings light or humidity', () => {
    const data = new PiDataSession({ sensorType: 'fire' }); 
    const { errors } = data.validateSync();

    expect(errors.sensorType.message).toEqual('`fire` is not a valid enum value for path `sensorType`.');
  });
